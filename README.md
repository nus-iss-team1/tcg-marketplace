...
The application runs on AWS using a serverless-first approach with ECS Fargate, fronted by API Gateway and an internal Application Load Balancer.
...
+=====================================================+
          |              |              |
     DynamoDB        S3 + CDN       Cognito
```

### Infrastructure

Infrastructure is managed with AWS CloudFormation across 5 stacks:

| Stack | Purpose | Key Resources |
|-------|---------|---------------|
| **01-foundation** | Networking and entry point | VPC, subnets (2 AZs), dual NAT Gateways, internal ALB, API Gateway with VPC Link |
| **02-security** | Container registries | ECR repositories for backend and frontend images |
| **03-data** | Storage and databases | DynamoDB tables (GameCardLookup, MessagingPlatform, TCGMarketplace), S3 bucket with CloudFront CDN |
| **04-services** | Compute | ECS Fargate task definitions and services, IAM roles, CloudWatch log groups |
| **05-auth** | Authentication | Cognito User Pool with email verification, admin/user groups, super admin bootstrap |

### CI/CD Pipeline

Both frontend and backend pipelines are triggered on push to `develop` via GitHub Actions:

```
Push to develop
    --> Lint (ESLint, zero warnings)
    --> Unit Tests (frontend only)
    --> Security Scan (Snyk, high severity threshold)
    --> Build and Push Docker Image to ECR
    --> Deploy to ECS Fargate (wait for stability)
```

Each image is tagged with both a rolling `develop-latest` tag and a unique `develop-<commit-sha>` tag for traceability. Deploy jobs run in separate concurrency groups to prevent mid-rollout cancellation.
