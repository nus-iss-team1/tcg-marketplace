...
if ($storageStatus -ne "CREATE_COMPLETE" -and $storageStatus -ne "UPDATE_COMPLETE") {
    Write-Host "WARNING: Storage stack not found or not ready. Your data might not be available." -ForegroundColor Yellow
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($dynamoTable) { Write-Host "DynamoDB Table: $dynamoTable" -ForegroundColor Cyan }
    
    Write-Host "Daily cost: approximately 1.58 dollars (remember to run dev-stop.ps1 tonight!)" -ForegroundColor Yellow
} else {
