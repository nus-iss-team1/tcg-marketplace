...
# pagination doesn't work so load all records in a page
page_size = 1_000_000
...
for game_name, base_url in games.items():
  print(f"\nFetching all cards for {game_name}...")
  page = 1
  data_list = []
  ...
  # save cards in separate JSON files
  filename = os.path.join(output_folder, f"{game_name.lower().replace(' ', '_').replace('!', '').replace(':', '')}.json")
  with open(filename, "w") as f:
    json.dump(data_list, f, indent=2)

  print(f"Saved {len(data_list)} records for {game_name} to {filename}")

print("\nDone! All game datasets have been saved separately.")
