...
function SellerSection({ sellerId, onMessage }: SellerSectionProps) {
  const [seller, setSeller] = useState<SellerProfile | null>(null);

  useEffect(() => {
    fetchSellerProfile(sellerId).then(setSeller);
  }, [sellerId]);

  if (!seller) return null;

  const displayName = seller.displayName || sellerId;
    ...
    <>
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-muted-foreground normal-case">@{seller.username}</p>
        </div>
      ...
      <div className="flex flex-1 flex-col lg:flex-row gap-6 lg:gap-16 h-full">
        ...
        <div className="flex-1 min-w-0 flex flex-col">
            {chatOpen ? (
              ...
              <div key="details" className="flex-1 flex flex-col animate-[fade-up_0.3s_ease-out_both]">
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isOwner && (
                    <Button variant="outline" size="sm" asChild>
                      <Link
                        href={`/listing/${listing.listingId}?game=${encodeURIComponent(listing.gameName)}&edit=true`}
                      >
                        Edit
                      </Link>
                    </Button>
                ...
                <h1 className="text-3xl sm:text-4xl font-heading">
                  {listing.listingStatus && (
                    <Badge
                      className="mr-3 align-middle text-2xl px-2 py-0.1"
                      variant={
                ...
                <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                  <Link
                    ...
                    className="underline hover:text-foreground transition-colors normal-case"
                  >
                    @{sellerId}
                  </Link>
                  <span>&middot;</span>
...
function EditListingView({ listing }: { listing: Listing }) {
  ...
  const canSubmit =
    ...
    price &&
    Number(price) > 0 &&
    (paymentMethod.cash || paymentMethod.paynow || paymentMethod.bank) &&
    !submitting;
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (uploadTarget === "front") {
      if (frontPreview && !frontPreview.startsWith("http"))
        URL.revokeObjectURL(frontPreview);
      setFrontImage(file);
      setFrontPreview(previewUrl);
    } else {
      if (backPreview && !backPreview.startsWith("http"))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const frontImageAction = frontImage
        ? ("REPLACE" as const)
        : ("KEEP" as const);
          ...
          {frontPreview ? (
            <div className="w-48 sm:w-56 md:w-64 shrink-0 aspect-5/7 rounded-none bg-background overflow-hidden snap-start relative group">
              ...
              <div className="absolute top-2 right-2 flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                <button
                  type="button"
                  className="h-7 w-7 flex items-center justify-center rounded-full bg-white/90 hover:bg-white text-black shadow-sm transition-colors"
          ...
          {backPreview ? (
            <div className="w-48 sm:w-56 md:w-64 shrink-0 aspect-5/7 rounded-none bg-background overflow-hidden snap-start relative group">
              ...
              <div className="absolute top-2 right-2 flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                ...
                <button
                  type="button"
                  className="h-7 w-7 flex items-center justify-center rounded-full bg-red-500/90 hover:bg-red-500 text-white shadow-sm transition-colors"
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2 h-9 items-center">
              <label className="flex items-center gap-2 text-sm">
                <Checkbox
                  ...
                  onCheckedChange={(v) =>
                    setPaymentMethod((p) => ({ ...p, cash: v === true }))
                  }
                />
                Cash
              </label>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox
                  ...
                  onCheckedChange={(v) =>
                    setPaymentMethod((p) => ({ ...p, paynow: v === true }))
                  }
                />
                PayNow
              </label>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox
                  ...
                  onCheckedChange={(v) =>
                    setPaymentMethod((p) => ({ ...p, bank: v === true }))
                  }
                />
                Bank Transfer
              </label>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Select an image of the{" "}
              {uploadTarget === "front" ? "front" : "back"} of your card.
...
function ImageCarousel({
  ...
  attachment?: { front?: string; back?: string; images?: string[] };
}) {
  ...
  if (images.length === 0) {
    return <ImagePlaceholder className="w-full aspect-5/7 rounded-none" />;
  }
